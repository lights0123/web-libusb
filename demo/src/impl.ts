...
export type BulkTransferOut = { device: number; endpoint: number; data: Uint8Array };

export type BulkTransferOutReply = { Ok: number } | { Err: UsbError };

export type BulkTransferIn = { device: number; endpoint: number; length: number };
...
export type USBAlternateInterface = { alternateSetting: number; interfaceClass: number; interfaceSubclass: number; interfaceProtocol: number; endpoints: USBEndpoint [] };

export type USBConfiguration = { configurationValue: number; interfaces: USBAlternateInterface [] [] };

export type ActiveConfigDescriptorReply = { Ok: USBConfiguration } | { Err: UsbError };
	...
	private async _processCmd(cmd: Cmd) {
		try {
			...
			if (cmd.usbCmd === 'bulkTransferOut') {
				const res = await this.devices[cmd.device].transferOut(cmd.endpoint & ~0x80, cmd.data);
				const reply: BulkTransferOutReply = { Ok: res.bytesWritten };
				return reply;
			} else if (cmd.usbCmd === 'bulkTransferIn') {
				const res = await this.devices[cmd.device].transferIn(cmd.endpoint & ~0x80, cmd.length);
				const reply: BulkTransferInReply = { Ok: new Uint8Array(res.data!.buffer) };
				return reply;
						...
						interfaces: configuration.interfaces.map(({ alternates }) => {
							return alternates.map(alternate => ({
								...
								endpoints: alternate.endpoints.map(endpoint => ({
									address: endpoint.endpointNumber | (endpoint.direction === 'in' ? 0x80 : 0),
