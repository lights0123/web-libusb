...
pub unsafe extern "C" fn free(p: *mut c_void) {
    ...
    alloc::dealloc(ptr as *mut u8, Layout::from_size_align_unchecked(size, ALIGN))
}
...
pub unsafe extern "C" fn realloc(p: *mut c_void, new_size: usize) -> *mut c_void {
    if p.is_null() {
        ...
        let new_ptr = alloc::realloc(ptr as *mut u8, Layout::from_size_align_unchecked(old_size, ALIGN), new_size + 8);
        if new_ptr.is_null() {
