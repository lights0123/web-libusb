...
pub fn init(id: u32, usb_id: (u16, u16), comm: Int32Array) -> *mut libusb_device_handle {
    let device = Box::into_raw(Box::new(libusb_device { id, usb_id, comm, refs: 0 }));
...
#[no_mangle]
pub unsafe extern "C" fn libusb_get_device(dev_handle: *mut libusb_device_handle) -> *mut libusb_device {
    (*dev_handle).device
}

#[no_mangle]
pub unsafe extern "C" fn libusb_bulk_transfer(handle: *mut libusb_device_handle, endpoint: u8, data: *mut u8, length: c_int, transferred: *mut c_int, timeout: c_uint) -> c_int {
    let device = (*(*handle).device).id;
    if endpoint & LIBUSB_ENDPOINT_DIR_MASK == LIBUSB_ENDPOINT_OUT {
        match REPLY_BUF.with(|buf| {
            let mut buf = buf.borrow_mut();
            call(&(*(*handle).device).comm, msgs::BulkTransferOut { device, endpoint, data: slice::from_raw_parts(data, length as usize) }, &mut buf)
        }) {
        ...
        REPLY_BUF.with(|buf| {
            let mut buf = buf.borrow_mut();
            match call(&(*(*handle).device).comm, msgs::BulkTransferIn { device, endpoint, length: length as usize }, &mut buf) {
                Ok(reply) => match reply.0 {
                    Ok(buf) => {
                        slice::from_raw_parts_mut(data, (buf.0).len().min(length as usize)).copy_from_slice(buf.0);
                        LIBUSB_SUCCESS
                    }
                    Err(e) => e.into()
                }
...
#[no_mangle]
pub unsafe extern "C" fn libusb_set_configuration(handle: *mut libusb_device_handle, config: c_int) -> c_int {
    let device = (*(*handle).device).id;
    match REPLY_BUF.with(|buf| {
        let mut buf = buf.borrow_mut();
        call(&(*(*handle).device).comm, msgs::SelectConfiguration { device, config: config as u8 }, &mut buf)
    }) {
        Ok(reply) => match reply.0 {
            Ok(()) => LIBUSB_SUCCESS,
            Err(e) => e.into()
...
#[no_mangle]
pub unsafe extern "C" fn libusb_claim_interface(handle: *mut libusb_device_handle, interface_number: c_int) -> c_int {
    let device = (*(*handle).device).id;
    match REPLY_BUF.with(|buf| {
        let mut buf = buf.borrow_mut();
        call(&(*(*handle).device).comm, msgs::ClaimInterface { device, number: interface_number as u8 }, &mut buf)
    }) {
        Ok(reply) => match reply.0 {
            Ok(()) => LIBUSB_SUCCESS,
            Err(e) => e.into()
...
#[no_mangle]
pub unsafe extern "C" fn libusb_release_interface(handle: *mut libusb_device_handle, interface_number: c_int) -> c_int {
    let device = (*(*handle).device).id;
    match REPLY_BUF.with(|buf| {
        let mut buf = buf.borrow_mut();
        call(&(*(*handle).device).comm, msgs::ReleaseInterface { device, number: interface_number as u8 }, &mut buf)
    }) {
        Ok(reply) => match reply.0 {
            Ok(()) => LIBUSB_SUCCESS,
            Err(e) => e.into()
...
pub unsafe extern "C" fn libusb_reset_device(handle: *mut libusb_device_handle) -> c_int {
    ...
    match REPLY_BUF.with(|buf| {
        let mut buf = buf.borrow_mut();
        call(&(*(*handle).device).comm, msgs::ResetDevice { device }, &mut buf)
    }) {
        Ok(reply) => match reply.0 {
            Ok(()) => LIBUSB_SUCCESS,
            Err(e) => e.into()
...
#[no_mangle]
pub unsafe extern "C" fn libusb_get_active_config_descriptor(handle: *const libusb_device, config_ptr: *mut *const libusb_config_descriptor) -> c_int {
    let device = (*handle).id;
    match REPLY_BUF.with(|buf| {
        let mut buf = buf.borrow_mut();
        call(&(*handle).comm, msgs::ActiveConfigDescriptor { device }, &mut buf)
    }) {
        Ok(reply) => match reply.0 {
            Ok(msg) => {
                let mut config: Box<libusb_config_descriptor> = Box::new(mem::zeroed());
                config.bConfigurationValue = msg.configuration_value;
                let interfaces: Vec<_> = msg.interfaces.into_iter().map(|interface| {
                    let settings: Vec<_> = interface.into_iter().map(|setting| {
                        let endpoints: Vec<_> = setting.endpoints.into_iter().map(|endpoint| {
...
#[no_mangle]
pub unsafe extern "C" fn libusb_free_config_descriptor(config_ptr: *const libusb_config_descriptor) {
    let config: Box<libusb_config_descriptor> = Box::from_raw(config_ptr as *mut _);
    let interfaces = Box::from_raw(&mut slice::from_raw_parts(config.interface, config.bNumInterfaces as usize));
    for interface in interfaces.iter() {
        let settings = Box::from_raw(&mut slice::from_raw_parts(interface.altsetting, interface.num_altsetting as usize));
        for setting in settings.iter() {
            Box::from_raw(&mut slice::from_raw_parts(setting.endpoint, setting.bNumEndpoints as usize));
        }
    }
}
